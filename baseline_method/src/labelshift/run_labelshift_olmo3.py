...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Label-shift mixture estimation for OLMo3 via domain classifier + prior correction"
    )
    # Data
    p.add_argument("--local_samples_dir", type=str, default=str(Path(__file__).resolve().parents[2] / "data_samples"))
    # Remove merge_web since it's not needed for OLMo3
    ...
    p.add_argument("--run_name", type=str, default="labelshift_olmo3")
    return p.parse_args()
...
def main() -> None:
    ...
    # For OLMo3, we assume the data loading logic in data_utils handles detecting the OLMo3 files
    # and we don't need to pass merge_web=True (which is for LLaMA 6-class).
    ds = build_balanced_splits(
        local_dir=args.local_samples_dir,
        merge_web=False, # Explicitly false for OLMo3
