"""
Neighborhood perturbation sensitivity with Binary Thresholding (StarCoder taxonomy)

Adapts the Neighborhood MIA signal (Mattern et al., ACL 2023) to StarCoder languages.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Estimate language mixture via Neighborhood perturbation sensitivity (StarCoder taxonomy)"
    )
    # Data
    p.add_argument(
        "--local_samples_dir",
    ...
    p.add_argument("--allow_missing", action="store_true", help="Proceed if some languages missing")
    ...
    p.add_argument("--run_name", type=str, default="neighborhood_threshold_starcoder")
    return p.parse_args()
...
def main() -> None:
    ...
    if args.fill_strategy == "global":
        ...
        print("Fill strategy: leave_one_out")
    ...
    payload = {
        "config": {
            ...
            "min_fill_vocab": args.min_fill_vocab,
            "threshold": threshold,
    ...
    print(f"\nWrote StarCoder Neighborhood outputs to {out_dir}")
