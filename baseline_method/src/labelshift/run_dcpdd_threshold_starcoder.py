"""
DC-PDD with Binary Thresholding for Language Composition Estimation (StarCoder taxonomy)

Adapts DC-PDD (Zhang et al., 2024) to the StarCoder language set.
score(x) = - mean_i [ p_i * log(1 / f(tok_i)) ] (clipped at a)
Default member rule: score <= threshold => member (member_if=lt).
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Estimate language mixture via DC-PDD with binary thresholding (StarCoder taxonomy)"
    )
    # Data
    p.add_argument(
        "--local_samples_dir",
    ...
    p.add_argument("--run_name", type=str, default="dcpdd_threshold_starcoder")
    return p.parse_args()
...
def main() -> None:
    ...
    print(f"\nWrote StarCoder DC-PDD outputs to {out_dir}")
