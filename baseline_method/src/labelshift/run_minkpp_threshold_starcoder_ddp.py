...
Usage:
    torchrun --nproc_per_node=NUM_GPUS run_minkpp_threshold_starcoder_ddp.py [args]
    
Example:
    torchrun --nproc_per_node=4 run_minkpp_threshold_starcoder_ddp.py \
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--run_name", type=str, default="minkpp_threshold_starcoder_ddp")
...
@torch.no_grad()
def minkpp_score(text: str, model, tok, device: str, max_tokens: int, ratio: float) -> float:
    """
...
def main() -> None:
    ...
    for lang, text, global_idx in pbar:
        score = minkpp_score(text, model, tok, device, args.max_tokens, args.mink_ratio)
