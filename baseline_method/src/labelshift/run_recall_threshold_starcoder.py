"""
ReCaLL with Binary Thresholding for Language Composition Estimation (StarCoder taxonomy)

Adapts ReCaLL (conditional vs unconditional likelihood ratio) to the StarCoder language set.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Estimate language mixture via ReCaLL with binary thresholding (StarCoder taxonomy)"
    )
    # Data
    p.add_argument(
        "--local_samples_dir",
    ...
    p.add_argument("--allow_missing", action="store_true", help="Proceed with subset if some languages missing")
    ...
    p.add_argument("--run_name", type=str, default="recall_threshold_starcoder")
    return p.parse_args()
...
def main() -> None:
    ...
    print(f"\nWrote StarCoder ReCaLL outputs to {out_dir}")
