"""
DUCI (Dataset Usage Cardinality Inference) for Category Composition Estimation (Pythia Taxonomy)

This script applies the DUCI methodology to estimate the proportion of each category
from the Pile dataset (Pythia taxonomy: ~17 categories) used in training an LLM.

Key Steps:
1. Isolate Target Datasets - Treat each category as an independent inference problem
2. Run MIA for each category - Using loss-based or Min-K-style signals
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="DUCI-based category composition estimation for LLM pretraining data (Pythia taxonomy)"
    )
    # Data (Pile-style)
    p.add_argument(
        "--local_samples_dir",
    ...
    # Reference model (for TPR/FPR estimation)
    p.add_argument("--reference_model", type=str, default=None,
                   help="Reference model for TPR/FPR calibration. If None, uses cross-validation.")
    ...
    p.add_argument("--run_name", type=str, default="duci_categories_pythia")
...
def main() -> None:
    ...
    print("=" * 70)
    print("DUCI: Dataset Usage Cardinality Inference (Pythia Taxonomy)")
    print("=" * 70)
    
    # Load category samples (Pythia taxonomy)
    print(f"\nLoading Pythia/Pile samples from: {args.local_samples_dir}")
    ...
    target_model, tok = load_model(
        args.target_model, revision=args.hf_revision,
        half=args.half, int8=args.int8, trust_remote_code=args.trust_remote_code
    )
    ...
    if args.reference_model:
        ...
        ref_model, _ = load_model(
            args.reference_model, revision=args.ref_revision,
            half=args.half, int8=args.int8, trust_remote_code=args.trust_remote_code
        )
    ...
    for cat in categories:
        ...
        stats = CategoryStats(
            category=cat, n_samples=len(scores), n_valid=n_valid, scores=scores,
            threshold=threshold, tpr=tpr, fpr=fpr,
            raw_member_rate=raw_rate, debiased_proportion=debiased_prop,
    ...
    # Normalize to get mixture
    total_debiased = sum(s.debiased_proportion for s in category_stats)
    
    print("\n" + "=" * 70)
    print("FINAL DUCI RESULTS: Estimated Category Proportions (Pythia Taxonomy)")
    print("=" * 70)
    
    global_mixture = {}
    for stats in category_stats:
        normalized = stats.debiased_proportion / total_debiased if total_debiased > 0 else 0.0
        global_mixture[stats.category] = normalized
        print(f"  {stats.category:20s}: {stats.debiased_proportion:.2%} (normalized: {normalized:.2%})")
